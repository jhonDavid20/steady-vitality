...
type PendingAction = { user: User; kind: "role" | "status"; value: Role | boolean };
...
export function AdminUsers() {
  ...
  async function confirmAction() {
    if (!pending) return;
    setSaving(true);
    const path = pending.kind === "role" ? `/api/marketplace/admin/users/${pending.user.id}/role` : `/api/marketplace/admin/users/${pending.user.id}/status`;
    const body = pending.kind === "role" ? { role: pending.value } : { isActive: pending.value };
    try {
  ...
  return <section className="space-y-5">
    ...
    {!loading && !error && visibleUsers.length > 0 ? <div className="overflow-x-auto rounded-xl border bg-card"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-3 font-medium">{t("user")}</th><th className="px-4 py-3 font-medium">{t("role")}</th><th className="px-4 py-3 font-medium">{t("status")}</th><th className="px-4 py-3 font-medium">{t("lastAccess")}</th><th className="px-4 py-3 font-medium"><span className="sr-only">{t("actions")}</span></th></tr></thead><tbody>{visibleUsers.map((user) => <tr className="border-b last:border-0" key={user.id}><td className="px-4 py-4"><p className="font-medium">{user.firstName} {user.lastName}</p><p className="text-muted-foreground">{user.email}</p></td><td className="px-4 py-4"><Select disabled={user.role === "admin"} onValueChange={(value) => setPending({ user, kind: "role", value: value as Role })} value={user.role}><SelectTrigger className="w-32"><SelectValue /></SelectTrigger><SelectContent>{roleOptions.map((option) => <SelectItem key={option} value={option}>{t(`roles.${option}`)}</SelectItem>)}</SelectContent></Select></td><td className="px-4 py-4"><span className={user.isActive ? "rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-800" : "rounded-full bg-destructive/10 px-2.5 py-1 text-xs font-medium text-destructive"}>{user.isActive ? t("active") : t("blocked")}</span></td><td className="px-4 py-4 text-muted-foreground">{user.lastLoginAt ? new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(new Date(user.lastLoginAt)) : t("never")}</td><td className="px-4 py-4 text-right"><Button disabled={user.role === "admin"} onClick={() => setPending({ user, kind: "status", value: !user.isActive })} size="sm" variant={user.isActive ? "outline" : "default"}>{user.isActive ? t("block") : t("unblock")}</Button></td></tr>)}</tbody></table></div> : null}
    <Dialog onOpenChange={(open) => { if (!open) setPending(undefined); }} open={Boolean(pending)}><DialogContent><DialogHeader><DialogTitle>{pending?.kind === "role" ? t("confirmRoleTitle") : pending?.value ? t("confirmUnblockTitle") : t("confirmBlockTitle")}</DialogTitle><DialogDescription>{pending?.kind === "role" ? t("confirmRoleDescription", { name: `${pending.user.firstName} ${pending.user.lastName}`, role: pending ? t(`roles.${pending.value as Role}`) : "" }) : t("confirmStatusDescription", { name: `${pending?.user.firstName ?? ""} ${pending?.user.lastName ?? ""}` })}</DialogDescription></DialogHeader><DialogFooter><Button disabled={saving} onClick={() => setPending(undefined)} variant="outline">{t("cancel")}</Button><Button disabled={saving} onClick={() => void confirmAction()} variant={pending?.kind === "status" && pending.value === false ? "destructive" : "default"}>{saving ? t("saving") : t("confirm")}</Button></DialogFooter></DialogContent></Dialog>
  </section>;
}
