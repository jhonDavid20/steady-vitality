...
export function CoachPlanBuilder() {
  ...
  return <div className="grid gap-8 lg:grid-cols-3">
    <form className="space-y-3" onSubmit={(event) => { const form = new FormData(event.currentTarget); void submit(event, "/api/workouts/exercises", { name: form.get("name"), muscleGroup: form.get("muscleGroup"), videoUrl: form.get("videoUrl") || null }); }}><h2 className="font-semibold">{t("newExercise")}</h2><Input name="name" required placeholder={t("exerciseName")} /><Input name="muscleGroup" required placeholder={t("muscleGroup")} /><Input name="videoUrl" type="url" placeholder={t("videoUrl")} /><Button>{t("create")}</Button></form>
