...
import type { NutritionPlanItem } from './NutritionPlan';

@Entity('nutrition_assignments')
@Index(['clientId'])
@Index(['coachId'])
export class NutritionAssignment {
  ...
  @Column({ type: 'jsonb' }) items: NutritionPlanItem[];
  @Column({ type: 'int' }) waterTargetMl: number;
